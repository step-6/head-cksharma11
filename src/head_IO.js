const parseInputs = function(inputs) {
  ...
  const isNumberOption = input =>
    isOptionSpecified(input) && isFinite(input[1]);

  const isOptionWithCount = input =>
    isOptionSpecified(input) && input.length > 2;

  const isOptionWithoutCount = input =>
    isOptionSpecified(input) && input.length == 2;
